...
function getCheckinStatus(response, checkin) {
    ...
    $.get(remoteURL + '/api/mecheckedin_online', response, function (data) {
        ...
        if (data.found) {
            ...
            $('#checkin').addClass('btn-danger').attr('disabled', 'disabled').html("<span class='glyphicon glyphicon-map-marker' aria-hidden='true'></span> You are checked in for next " + data.till + " minutes");
